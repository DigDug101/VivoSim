 ...
 *
 *  VIVOSIM FARM HUD - Main script
*/

string TXT_TESTER = "TESTER";     // For testing purposes

float  VERSION = 6.00;          // 23 March 2023
string subVersion = "RC";       // Set to "" for release
...
integer timeRate =  90 * 60;                // EAT_INTERVAL         Default is 90 i.e. eat approx every 90 minutes
...
list    screenLines;
list    lastFoundKeys;
integer lastTs;				// Used to control increase/decrease of health, drunkeness etc
...
float   accumulatedTime;	// As script time resets when we detach/attach HUD, use this to carry over
string  lastNetCheck;
string  status;
string  lookingFor;
integer itemFound;
...
loadStoredVals()
{
	if (safetyCheck == FALSE)
	{
		...
		if (llGetListLength(descValues) >0)
		{
			...
			accumulatedTime = llList2Float(descValues, 25);

			// Fastest allowed is eating every 30 minutes
			if (timeRate <30) timeRate = 30;
			// We store in description as minutes value so convert to seconds
			timeRate = timeRate * 60;
...
string vals2Desc()
{
	...
	if (safetyCheck == FALSE)
	{
		result = "H;" +(string)llRound(hungry) +";" +(string)llRound(thirsty) +";" +(string)llRound(hygiene) +";" +(string)llRound(health) +";"
			...
			+privKey+ ";" +(string)provLocked +";" +(string)woodStore +";" +(string)llRound(timeRate/60) +";"
			+(string)healthMode +";" +llRound(energy) +";" +(string)OOC +";" +languageCode +";"
			+(string)hudSpacing +";" +(string)echoChat +";" +(string)statusHudVisible +";" +(string)energyStore +";" +(string)llRound(accumulatedTime);
...
refresh()
{
	...
	if (active == TRUE)
	{
		if (sleeping == FALSE)
		{
			...
			// Elapsed time from lase zeroing of script timer, plus any we already accumlated from previous sessions
			float elapsedTime = llGetTime() + accumulatedTime;

//float giveXpTarget = 1.5 * timeRate * 60;
			...
			// Only need to check every so often
			if (elapsedTime > timeRate * 2)
			{

float remainingDuration = timeRate - elapsedTime;
float remainingPoints = 100 - point;
float points_per_interval = (remainingPoints) / (remainingDuration / 10.0); // Divide the remaining duration into 10-second intervals
float points_to_add = points_per_interval * llRound(elapsedTime / 10.0) - point;

hungry += llRound(points_to_add);
// hungry += (elapsedTime/60.0) * (100/timeRate);

hungry -= llRound(points_to_add);
// thirsty += (elapsedTime/60.0) * (100/timeRate);
				...
				{
					// Award points based on the elapsed time
					point += llRound(points_to_add);

					if (point > 100) point = 100.0;

					// Just living uses energy so take a bit off!
					energy -=1;
				}
				...
				if (healthMode == TRUE)
				{
					...
					llOwnerSay("elapsedTime="+(string)elapsedTime +"hungry="+(string)hungry + " bladder="+(string)bladder +" point="+(string)point);
			...
			if (indicatorKey != NULL_KEY)
			{
				...
				else
				{
					...
					debug("Hygiene="+llRound(hygiene) +": Bladder="+llRound(bladder) +" : Health="+llRound(health) +" : Drunk="+llRound(drunk) +" : Energy="+llRound(energy) +"\nWellbeing="+ (string)wellbeing);
llOwnerSay("Hygiene="+llRound(hygiene) +": Bladder="+llRound(bladder) +" : Health="+llRound(health) +" : Drunk="+llRound(drunk) +" : Energy="+llRound(energy) +"\nWellbeing="+ (string)wellbeing);
...
integer handleValues(list tok, integer consume, integer apply)
{
	...
	float   totalValue = 100.0 / llList2Float(tok, 1);
...
{
	if (healthMode == TRUE) checkIndicator(statusIndicator);
	refresh();
	string str = "";
...
systemReset()
{
	...
	// Now set sensible default values
	drunk   = 0.0;
	thirsty = 0.0;
	hungry  = 0.0;
	health = 100.0;
...
setPaused()
{
   ...
   if (checkIndicator(statusIndicator) == TRUE)  messageObj(indicatorKey, "VISIBILITY|" +PASSWORD+ "|0");
   refresh();
}
...
startUp()
{
	...
	if (checkIndicator(statusIndicator) == TRUE)  messageObj(indicatorKey, "VISIBILITY|" +PASSWORD+ "|"+(string)statusHudVisible);
...
default
{
	...
	listen(integer c, string nm, key id, string m)
	{
		...
		else
		{
			...
			else if (m == TXT_RESUME)
			{
				...
				if (checkIndicator(statusIndicator) == TRUE)  messageObj(indicatorKey, "VISIBILITY|" +PASSWORD+ "|1");
				refresh();
			}
			...
			else if (m == "-"+TXT_DEBUG)
			{
				...
				if (checkIndicator(statusIndicator) == TRUE) messageObj(indicatorKey, "DEBUG|" +PASSWORD +"|0");
				refresh();
			}
			else if (m == "+"+TXT_DEBUG)
			{
				...
				if (checkIndicator(statusIndicator) == TRUE) messageObj(indicatorKey, "DEBUG|" +PASSWORD +"|1");
				refresh();
			}
	...
	http_response(key request_id, integer httpstatus, list metadata, string body)
	{
		if (request_id != req_id2)
		{
		  ...
		  // debug("http_response: id not matching (" +body +")");
		}
		else
		{
			if (httpstatus == 200)
	...
	state_entry()
	{
		...
		else
		{
			...
			if ((objDesc == "") || (objDesc == "*") || (llGetSubString(objDesc, 0, 0) != "H"))
			{
				systemReset();
			}
			else
