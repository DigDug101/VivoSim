...
float  VERSION = 6.02;          // 28 March 2023
...
integer timeRate = 5400;                    // EAT_INTERVAL         Default eat approx every 90 minutes i.e. 5400 seconds
string  timeRateDisp = "90";                //                      This is the minutes value we use in displays
...
setConfig(string line)
{
    ...
    if (llList2String(tok,1) != "")
    {
        ...
        else if (cmd == "EAT_INTERVAL")
        {
            ...
            // Set the display value to be shown as minutes
            timeRateDisp = (string)timeRate;

            // Now convert actual value into seconds
...
string vals2Desc()
{
    ...
    if (safetyCheck == FALSE)
    {
        result = "H;" +(string)llRound(hungry) +";" +(string)llRound(thirsty) +";" +(string)llRound(hygiene) +";" +(string)llRound(health) +";"
            ...
            +privKey+ ";" +(string)provLocked +";" +(string)woodStore +";" +timeRateDisp +";"
            +(string)healthMode +";" +llRound(energy) +";" +(string)OOC +";" +languageCode +";"
            +(string)hudSpacing +";" +(string)echoChat +";" +(string)statusHudVisible +";" +(string)energyStore +";" +(string)llRound(elapsedTime);
...
refresh()
{
    ...
    if (active == TRUE)
    {
        if (sleeping == FALSE)
        {
            ...
            // Round the elapsedTime variable to the nearest whole number of seconds
            elapsedTime = llRound(elapsedTime);

            // Calculate the points_to_add variable
            float points_to_add = (100 - point) / (timeRate - elapsedTime);

            if ((points_to_add > 0.9) || (points_to_add < 0.0)) points_to_add = 0.5;

            hungry += points_to_add;

            if (hungry > 100.0) hungry = 100.0;

            if (points_to_add > 0.0) thirsty += points_to_add/1.5;
            ...
            else
            {
                ...
                // Just living uses energy so take a little off
                energy -= points_to_add / 50;
            ...
            if (hungry > trigLevel)
            {
                ...
                {
                     hungry = 90;
                    health -=0.5;  if (health <1.0) health = 1.0;
                }
            }
            else if (hungry<5)
            {
...
systemReset()
{
    ...
    woodStore = 0;
    energyStore = 0;
    timeRate = 5400;        // 90 minutes which is 5400 seconds
    timeRateDisp = "90";
    fulfilment = 0;
    frustration = 0;
    userXP = -1;
    ...
    listen(integer c, string nm, key id, string m)
    {
        ...
        else
        {
            ...
            else if (m == TXT_OPTIONS)
            {
                ...
                info_txt += "\t" +TXT_CONSUME_INTERVAL+": "+ timeRateDisp+" "+TXT_MINUTES+"\n \n" + TXT_HEALTH_MODE+": ";
            ...
            else if (m == TXT_CONSUME_INTERVAL)
            {
                ...
                llTextBox(id, TXT_CONSUME_INTERVAL +" (" +TXT_MINUTES +")\n(" + TXT_CURRENT_VALUE +": " +timeRateDisp +" " +TXT_MINUTES +")", chan(llGetKey()));
            }
            ...
            else if (status =="waitConsumeInterval")
            {
                status = "";
                integer tmpVal = (integer)m;
                if (tmpVal <30) timeRate = 30; else timeRate = tmpVal;
                timeRateDisp = (string)timeRate;
                refresh();
                floatText(TXT_CONSUME_INTERVAL +": " +timeRateDisp + "min" +"\n ", PURPLE, 1);
            }
...
state paused
{
    ...
    touch_start(integer num_detected)
    {
        ...
        if (checkIndicator(statusIndicator) == TRUE)
        {
            messageObj(indicatorKey, "VISIBILITY|" +PASSWORD+ "|" +(string)statusHudVisible);
