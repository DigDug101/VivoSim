 ...
 * CHANGE LOG
 *  Now connects to vivosim.net server
 *  Uses the profile page cover image rather than an image file
 *  Display font can be set via config notecard [note that the font must be installed on the OpenSim server for that region]
 *  Fixed that it wasn't offering the language change option
 *  Fixed dark/light mode reset to light on script reset
...
string ImageURL_W = "vivosim.net/images/logo/vexw.png";   // for white background
string ImageURL_B = "vivosim.net/images/logo/vexb.png";   // for black background
string BASEURL;
string BASEIMAGEURL;
...
loadConfig()
{
    ...
    if (useHTTPS == TRUE)
    {
        if (useBeta == TRUE)
        {
            ...
            if (useDarkMode == TRUE) BASEIMAGEURL = "https://beta."+ImageURL_B; else BASEIMAGEURL = "https://beta."+ImageURL_W;
        }
        else
        {
            BASEURL = "https://"+URL;
            if (useDarkMode == TRUE) BASEIMAGEURL = "https://"+ImageURL_B; else BASEIMAGEURL = "https://"+ImageURL_W;
        }
    }
    else
    {
        if (useBeta == TRUE)
        {
            ...
            if (useDarkMode == TRUE) BASEIMAGEURL = "http://beta."+ImageURL_B; else BASEIMAGEURL = "http://beta."+ImageURL_W;
        }
        else
        {
            BASEURL = "http://"+URL;
            if (useDarkMode == TRUE) BASEIMAGEURL = "http://"+ImageURL_B; else BASEIMAGEURL = "http://"+ImageURL_W;
        }
    }
...
setImage()
{
    if (profileCover != "")
    {
        // show the owners JomSocial profile cover image
        llMessageLinked(LINK_SET, 1, "SHOW_IMAGE|" +profileCover , "");
    }
    else
    {
        if (getLinkNum("secondary_display") != -1) llSetLinkTexture(getLinkNum("secondary_display"), vivoLogo, FACE);
        if (getLinkNum("display_board") != -1) llSetLinkTexture(getLinkNum("display_board"), vivoLogo, FACE);
    }
}
...
messageScreen(string msg)
{
    ...
    if (useScreen == TRUE)
    {
        ...
        if (quinActive == TRUE)
        {
            if (useDarkMode == TRUE) CommandList = osSetPenColor(CommandList, "cornsilk"); else CommandList = osSetPenColor(CommandList, "navy");
            ...
            if (useDarkMode == TRUE) CommandList = osSetPenColor(CommandList, "gold"); else CommandList = osSetPenColor(CommandList, "darkgreen");
...
showPrices()
{
    ...
    if (useScreen == TRUE)
    {
        ...
        // Show key
        if (useDarkMode == TRUE) CommandList = osSetPenColor(CommandList, "cornsilk"); else CommandList = osSetPenColor(CommandList, "MidnightBlue");
    ...
    listen(integer c, string nm, key id, string m)
    {
        ...
        if (c != FARM_CHANNEL)
        {
            ...
            else if (m == TXT_OPTIONS)
            {
                ...
                if (useDarkMode == TRUE) opts += ["-"+TXT_DARK_MODE]; else opts += ["+"+TXT_DARK_MODE];
                startListen();
                ...
                if (useDarkMode == TRUE) tmpStr +=TXT_ON; else tmpStr += TXT_OFF;
            ...
            {
                startListen();
                llTextBox(userToPay, TXT_EX_NAME, chan(llGetKey()));
                status = "getName";
                llSetTimerEvent(300);
            }
            ...
            else if ((m == "+"+TXT_DARK_MODE) || (m == "-"+TXT_DARK_MODE))
            {
                ...
                if (useDarkMode == TRUE) llOwnerSay(TXT_DARK_MODE +" " +TXT_ON); else llOwnerSay(TXT_DARK_MODE +" " +TXT_OFF);
    ...
    http_response(key request_id, integer HStatus, list metadata, string body)
    {
        ...
        if (request_id == farmHTTP)
        {
            ...
            else if (cmd == "IMAGE")
            {
                ...
                if (profileCover == "INVALID-A") profileCover = "";
                setImage();
            }
            else if (cmd == "MENU")
            {
    ...
    // Check if there is an ID for this Exchang
            ...
            else if (cmd == "SOLD")
            {
                ...
                else
                {
                    ...
                    {
                        llRegionSayTo(userToPay, 0, TXT_CREDITED + " " + llList2String(tok,3) + " " + TXT_COINS);
